...
## Amazon Sales Effect using Zero Miles Data
...
average_treatment_effect(cf1_ama, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATET")
...
m <- test_calibration(cf1_ama)

stargazer(m, dep.var.caption = "CATE")
...
average_treatment_effect(cf_all_ama, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATE")
...
average_treatment_effect(cf2_ama, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATE")
...
# Run best linear predictor analysis
test_calibration(cf2_ama) %>% stargazer()
...
Y1_bb <- bb_cf_d1$prod_totprice %>% log(.) %>% {.+1}
...
# Estimate ATE
alpha <- 1-0.90
average_treatment_effect(cf1_bb, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATE")
...
ATE1_bb = average_treatment_effect(cf1_bb, target.sample = "treated")
paste("90% CI for the ATE:", round(ATE1_bb[1], 3),
      "+/-", round(qnorm(0.95) * ATE1_bb[2], 3))

ATE1_bb = average_treatment_effect(cf1_bb, target.sample = "all")
...
## Best Buy Sales Effect using All Data
```{r}
set.seed(1)
...
W_all_bb <- bb_cf_all$treatment 

Y_all_bb <- bb_cf_all$prod_totprice %>% log(.) %>% {.+1}
...
all_bb_exp <-model.matrix(~.+0, all_bb)

X_all_bb <- cbind(bb_cf_all[,-c(4, 16, which(colnames(bb_cf_all) %in% colnames(all_bb)))], all_bb_exp)

Y_all_f_bb <- regression_forest(X_all_bb, Y_all_bb)
Y_all_hat_bb <- predict(Y_all_f_bb)$predictions

W_all_f_bb <- regression_forest(X_all_bb, W_all_bb)
W_all_hat_bb <- predict(W_all_f_bb)$predictions

cf_all_raw_bb <- causal_forest(X_all_bb, Y_all_bb, W_all_bb,
                       Y.hat = Y_all_hat_bb, W.hat = W_all_hat_bb)

varimp_all_bb <- variable_importance(cf_all_raw_bb)
selected_all_idx_bb <- which(varimp_all_bb > mean(varimp_all_bb))

cf_all_bb <- causal_forest(X_all_bb[,selected_all_idx_bb], Y_all_bb, W_all_bb,
                   Y.hat = Y_all_hat_bb, W.hat = W_all_hat_bb,
                   tune.parameters = "all")

tau_all_hat_bb <- predict(cf_all_bb)$predictions
...
ATE_all_bb = average_treatment_effect(cf_all_bb, target.sample = "all")
paste("90% CI for the ATE:", round(ATE_all_bb[1], 3),
      "+/-", round(qnorm(0.95) * ATE_all_bb[2], 3))
...
Y2_bb <- bb_cf_d2$prod_totprice %>% log(.) %>% {.+1}
...
ATE2_bb = average_treatment_effect(cf2_bb, target.sample = "all")
...
average_treatment_effect(cf3_ama, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATE")
...
### Amazon Pages Per Dollar using all Data
```{r}
set.seed(1)
...
W_all2_ama <- ama_cf_all2$treatment 

Y_all2_ama <- ama_cf_all2$PagesPerDollar %>% log(.) %>% {.+1}

all2_ama <- ama_cf_all2 %>% 
  select(-duration, -prod_qty, 
         -PagesPerDollar,
         -household_income, 
         -treatment)

all2_ama_exp <-model.matrix(~.+0, all2_ama)

X_all2_ama <- cbind(ama_cf_all2[,-c(14, 15, which(colnames(ama_cf_all2) %in% colnames(all2_ama)))], all2_ama_exp)

Y_all2_f_ama <- regression_forest(X_all2_ama, Y_all2_ama)
Y_all2_hat_ama <- predict(Y_all2_f_ama)$predictions

W_all2_f_ama <- regression_forest(X_all2_ama, W_all2_ama)
W_all2_hat_ama <- predict(W_all2_f_ama)$predictions

cf_all2_raw_ama <- causal_forest(X_all2_ama, Y_all2_ama, W_all2_ama,
                       Y.hat = Y_all2_hat_ama, W.hat = W_all2_hat_ama)

varimp_all2_ama <- variable_importance(cf_all2_raw_ama)
selected_all2_idx_ama <- which(varimp_all2_ama > mean(varimp_all2_ama))

cf_all2_ama <- causal_forest(X_all2_ama[,selected_all2_idx_ama], Y_all2_ama, W_all2_ama,
                   Y.hat = Y_all2_hat_ama, W.hat = W_all2_hat_ama,
                   tune.parameters = "all")

tau_all2_hat_ama <- predict(cf_all2_ama)$predictions
...
ATE_all2_ama <- average_treatment_effect(cf_all2_ama, target.sample = "all")
paste("95% CI for the ATE:", round(ATE_all2_ama[1], 3),
      "+/-", round(qnorm(0.975) * ATE_all2_ama[2], 3))
...
Y4_ama <- ama_cf_d4$PagesPerDollar %>% log(.) %>% {.+1}
...
average_treatment_effect(cf3_bb, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
...
average_treatment_effect(cf5_ama, target.sample = "treated") %>%
  {.["estimate"] + c(-1,0,1)*qnorm(1-alpha/2)*.["std.err"]} %>% 
  as.table() %>% t() %>% 
  set_colnames(c("5% ", "Estimate", "95%")) %>% 
  xtable(caption = "90% CI for the ATT")
