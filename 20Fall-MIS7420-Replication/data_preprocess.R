...
temp <- dta_bal_imp(data_0m_t4)

# playground
ama.t4.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = temp[temp$domain_name == "amazon.com", ], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.t4.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = temp[temp$domain_name == "bestbuy.com", ], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
