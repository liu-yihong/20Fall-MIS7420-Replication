...
# CEM
data_0m_t11_raw <- sqldf("SELECT Zip_Code, SUM(prod_totprice) AS TotalMonthlySales,
...
ziplist <- c()
for (i in 1:length(data_0m_t11_raw$Zip_Code)){
...
data_0m_t13_balanced <- dta_bal_imp_all(data_0m_t13[data_0m_t13$Zipmatch==1, ])

# Table 13
ama.t13.0mile.sale <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code"), model = "within")
ama.t13.pagesperdollar.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code"), model = "within")
ama.t13.minsperdollar.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code"), model = "within")

bb.t13.0mile.sale <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code"), model = "within")
bb.t13.pagesperdollar.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code"), model = "within")
bb.t13.minsperdollar.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code"), model = "within")

stargazer(ama.t13.0mile.sale, bb.t13.0mile.sale,
          ama.t13.pagesperdollar.0mile, bb.t13.pagesperdollar.0mile,
          ama.t13.minsperdollar.0mile, bb.t13.minsperdollar.0mile,
          ...
          column.labels=c("Amazon-0 Mile","BesyBuy-0 Mile","Amazon-0 Mile","BesyBuy-0 Mile","Amazon-0 Mile","BesyBuy-0 Mile"))
...
# Create Baseline
ama.0m.t14.sale.base <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.0m.t14.ppd.base  <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.0m.t14.mpd.base  <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

bb.0m.t14.sale.base <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.0m.t14.ppd.base  <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.0m.t14.mpd.base  <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_t13_balanced[(data_0m_t13_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
...
coeftest(bb.0m.t14.sale.base, vcovDC)
coeftest(bb.0m.t14.ppd.base, vcovDC)
coeftest(bb.0m.t14.mpd.base, vcovDC)

# 
stargazer(ama.0m.t14.sale.base, bb.0m.t14.sale.base,
          ama.0m.t14.ppd.base, bb.0m.t14.ppd.base,
          ama.0m.t14.mpd.base, bb.0m.t14.mpd.base,
