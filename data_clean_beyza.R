...
groupby_ref_domain_result <- aggregate(machine_id ~ ref_domain_name, 
                                       rbind(sales_allother_zipcode, sales_cc_0mile, sales_cc_5miles), 
                                       FUN = "length")
...
# Table 1
table1_raw <-  rbind(read_sas(sales_allother_zipcode_path, encoding = "latin1"), read_sas(sales_cc_0mile_path))
table1 <- sqldf("SELECT domain_name as DomainName, count(*) as TotalTransaction, 
                SUM(prod_totprice) AS TotalSales, SUM(pages_viewed) AS TotalPagesViewed, 
                SUM(pages_viewed)/SUM(prod_totprice) AS PagesPerDollar, SUM(duration) AS TotalDuration, 
                SUM(duration)/SUM(prod_totprice) AS MinsPerDollar FROM table1_raw GROUP BY domain_name ORDER BY TotalSales DESC")
...
table2_raw <-  rbind(read_sas(sales_allother_zipcode_path, encoding = "latin1"), read_sas(sales_cc_0mile_path))
...
ama.t4.0mile <- plm(log(TotalMonthlySales + 1) ~ 
                      CCStorePresent + AfterStoreClosing + BBStorePresent + CCStorePresent:AfterStoreClosing + 
                      CCStorePresent:AfterStoreClosing:BBStorePresent, 
                    data = data_0m_t4[data_0m_t4$domain_name == "amazon.com",], 
                    index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
...
data_0m_ama_cf <- data_0m_t4 %>% 
  mutate(W = {CCStorePresent*AfterStoreClosing}) %>% 
  group_by(Zip_Code, domain_name, CCStorePresent, BBStorePresent, W) %>% 
  summarise(TotalSales = sum(TotalMonthlySales)) %>% 
  mutate(BBStorePresent = as.factor(BBStorePresent),
         Zip_Code = as.factor(Zip_Code),
         CCStorePresent = as.factor(CCStorePresent)) %>% 
  filter(domain_name == "amazon.com") 
         
# w.lm = glm(W ~ ., data = data_0m_cf %>% 
#              select(Zip_Code, BBStorePresent, W), family = binomial)
# summary(w.lm)

W_ama = data_0m_ama_cf$W
Y_ama = data_0m_ama_cf$TotalSales
# X.raw = DF[,-(1:2)]

X_raw_ama <- data_0m_ama_cf[,"BBStorePresent", drop=TRUE] 
X_ama <-model.matrix(~X_raw_ama + 0)

Y_f_ama = regression_forest(X_ama, Y_ama)
Y_hat_ama = predict(Y_f_ama)$predictions
W_f_ama = regression_forest(X_ama, W_ama)
W_hat_ama = predict(W_f_ama)$predictions

cf_raw_ama = causal_forest(X_ama, Y_ama, W_ama,
                       Y.hat = Y_hat_ama, W.hat = W_hat_ama)

varimp_ama = variable_importance(cf_raw_ama)

selected_idx_ama = which(varimp_ama > mean(varimp_ama))

cf_ama = causal_forest(X_ama[,selected_idx_ama, drop=FALSE], Y_ama, W_ama,
                   Y.hat = Y_hat_ama, W.hat = W_hat_ama,
                   tune.parameters = "all")

tau_hat_ama = predict(cf_ama)$predictions

#
# Estimate ATE
#
ATE_ama = average_treatment_effect(cf_ama, target.sample = "treated")

paste("95% CI for the ATE:", round(ATE_ama[1], 3),
      "+/-", round(qnorm(0.975) * ATE_ama[2], 3))
