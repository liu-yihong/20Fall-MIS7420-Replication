...
dta_bal_imp_three <- function(unbalanced_data){
  balanced_data <- NA
  ...
  for (domain_name in unique(unbalanced_data$domain_name)) {
    ...
    temp <- temp %>%
      group_by(Zip_Code) %>%
      mutate(PagesPerTransaction = ifelse(is.na(PagesPerTransaction), mean(PagesPerTransaction, na.rm=TRUE), PagesPerTransaction))
    temp <- temp %>%
      group_by(Zip_Code) %>%
      mutate(MinsPerTransaction = ifelse(is.na(MinsPerTransaction), mean(MinsPerTransaction, na.rm=TRUE), MinsPerTransaction))
    temp <- temp %>%
      group_by(Zip_Code) %>%
      mutate(SalesPerTransaction = ifelse(is.na(SalesPerTransaction), mean(SalesPerTransaction, na.rm=TRUE), SalesPerTransaction))
...
data_0m_t10 <- dta_bal_imp_three(data_0m_t10)

# Table 10
ama.t10.0mile.SalesPerTransaction <- plm(log(SalesPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "amazon.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.t10.0mile.SalesPerTransaction  <- plm(log(SalesPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "bestbuy.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.t10.0mile.PagesPerTransaction <- plm(log(PagesPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "amazon.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.t10.0mile.PagesPerTransaction  <- plm(log(PagesPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "bestbuy.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.t10.0mile.MinsPerTransaction <- plm(log(MinsPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "amazon.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.t10.0mile.MinsPerTransaction  <- plm(log(MinsPerTransaction + 1) ~ DID + THREEINTER, data = data_0m_t10[data_0m_t10$domain_name == "bestbuy.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
