...
data_0m_tE_balanced <- dta_bal_imp_all(data_0m_tE)

# Table E1
staple.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "staples.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
walmart.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "walmart.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
dell.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "dell.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
cc.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "circuitcity.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

stargazer(staple.tE1.0mile, walmart.tE1.0mile, dell.tE1.0mile, cc.tE1.0mile,
          title="Results of the Sales Effect (All Product Categories)",
          align=TRUE, column.labels=c("staples.com-0 Mile","walmart.com-0 Mile","dell.com-0 Mile", "circuitcity.com-0 Mile"),
          covariate.labels=c("beta_1", "beta_2"), no.space=TRUE, column.sep.width = "1pt", label = "tab:tabE1")

# Table E2
staple.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "staples.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
walmart.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "walmart.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
dell.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "dell.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
cc.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "circuitcity.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

stargazer(staple.tE2.0mile, walmart.tE2.0mile, dell.tE2.0mile, cc.tE2.0mile,
          title="Results of the Online Search Effect (All Product Categories)",
          align=TRUE, column.labels=c("staples.com-0 Mile","walmart.com-0 Mile","dell.com-0 Mile", "circuitcity.com-0 Mile"),
          covariate.labels=c("beta_1", "beta_2"), no.space=TRUE, column.sep.width = "1pt", label = "tab:tabE2")

# Table E3
staple.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "staples.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
walmart.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "walmart.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
dell.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "dell.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
cc.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_tE_balanced[data_0m_tE_balanced$domain_name == "circuitcity.com",], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

stargazer(staple.tE3.0mile, walmart.tE3.0mile, dell.tE3.0mile, cc.tE3.0mile,
          title="Results of the Online Search Effect (All Product Categories)",
          align=TRUE, column.labels=c("staples.com-0 Mile","walmart.com-0 Mile","dell.com-0 Mile", "circuitcity.com-0 Mile"),
          covariate.labels=c("beta_1", "beta_2"), no.space=TRUE, column.sep.width = "1pt", label = "tab:tabE3")
