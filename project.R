...
#matching based on zipcode demographics 
...
data_0m_t11 <- data.frame(na.omit(data_0m_t11))
...
mat <- cem(treatment = "CCStorePresent", data = data_0m_t11, drop = "TotalMonthlySales", k2k ="True")


# We got 110 zipcodes in total. We checked 2 dataframe from CEM results, "w" and "matched", and both have 110 values.
# Fortunately, they are the same. In the future, just use data from "matched". 

# assign zipcode from "matched"
ziplist <- c()

for (i in 1:length(mat$matched)){
  if (mat$matched[i] == "TRUE") ziplist <-c(ziplist,i)
}

data.frame(ziplist)

# assign zipcode from "w"
ziplist1 <- c()

for (i in 1:length(mat$w)){
  if (mat$w[i] == 1) ziplist1 <-c(ziplist1,i)
}

data.frame(ziplist1)

# Test both dataframe, and they are same. 
all.equal(ziplist,ziplist1)

# assign matched zipcode to dataset
concat_data1$Zipmatch <- ifelse(concat_data1$Zip_Code %in% ziplist, 1, 0)

sum(concat_data1$Zipmatch == 1)
