...
# PSM
data_0m_psm_raw <- sqldf("SELECT Zip_Code, SUM(prod_totprice) AS TotalMonthlySales,
...
ps<- glm(CCStorePresent	~ HoHSize + HoHAge + HoHIncome + HoHChildren + HoHSpeed,	
                                  data	=data_0m_psm_raw,	family	= binomial())

summary(ps)

#Attach	the	predicted	propensity	score	to	the	datafile
data_0m_psm_raw$psvalue	<- predict(ps,	type	= "response")

#PSM 	histogram
library(MatchIt)
library("RItools")
library(Hmisc)
histbackback(split(data_0m_psm_raw$psvalue,	data_0m_psm_raw$CCStorePresent),	
             main= "Propensity_Score_b4_matching",	xlab=c("control",	"treatment"))

#---Match	using	near-neighbor
m.nn	<- matchit(CCStorePresent	~ HoHSize + HoHAge + HoHIncome + HoHChildren + HoHSpeed,	
                   data	=data_0m_psm_raw,	method= "nearest",	ratio	= 1)
summary(m.nn)
match_data	= match.data(m.nn)
plot(m.nn,	type	= "jitter")

histbackback(split(match_data$psvalue,match_data$CCStorePresent),	main= "Propensity_Score_After_Matching",	xlab=c("control",	"treatment"))


data_0m_psm <- sqldf("SELECT Zip_Code, MonthYear, domain_name, SUM(prod_totprice) AS TotalMonthlySales, SUM(pages_viewed) / SUM(prod_totprice) AS PagesPerDollar, SUM(duration) / SUM(prod_totprice) AS MinsPerDollar, AVG(CCStorePresent) AS CCStorePresent, AVG(BBStorePresent) AS BBStorePresent, AVG(AfterStoreClosing) AS AfterStoreClosing FROM concat_data1 GROUP BY Zip_Code, MonthYear, domain_name")
...
data_0m_psm_balanced <- dta_bal_imp_all(data_0m_psm)

# assign matched zipcode to dataset
data_0m_psm_balanced$Zipmatch <- ifelse(data_0m_psm_balanced$Zip_Code %in% match_data$Zip_Code, 1, 0)
data_0m_psm_balanced <- data_0m_psm_balanced[data_0m_psm_balanced$Zipmatch == 1, ]

# Table PSM
ama.psm.0mile.sale <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.psm.pagesperdollar.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
ama.psm.minsperdollar.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "amazon.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

bb.psm.0mile.sale <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.psm.pagesperdollar.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")
bb.psm.minsperdollar.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_psm_balanced[(data_0m_psm_balanced$domain_name == "bestbuy.com"),], index = c("Zip_Code", "Time"), model = "within", effect = "twoways")

stargazer(ama.psm.0mile.sale, bb.psm.0mile.sale,
          ama.psm.pagesperdollar.0mile, bb.psm.pagesperdollar.0mile,
          ama.psm.minsperdollar.0mile, bb.psm.minsperdollar.0mile,
          title="Results of the Online Sales and Search Effect After Propensity Score Matching: TotalMonthlySales, PagesPerDollar, and MinsPerDollar (All Product Categories)",
          align=TRUE, covariate.labels=c("beta_1", "beta_2"), no.space=TRUE,
          column.sep.width = "1pt", label = "tab:tablepsm",
