...
\newcommand{\hmwkAuthorName}{\textbf{Author: Beyza Celik, Luoying Chen, Duc Vu, Yihong Liu }}
\newcommand{\hmwkID}{\textbf{NETID: BXC190000, LXC190027, DDV110020, YXL180111 }}
%
% Title Page
%
...
\pagebreak
\subsection{Table 6}
We then test whether the show-rooming effect upon online sales is stronger for experience goods, by grouping products into physical experience goods and search goods. Table \ref{tab:table6} presents the results for sales model on physical experience products and search products.
\begin{table}[!h] \centering 
...
\pagebreak
\subsection{Table 7}
We then test whether the show-rooming effect upon online search behaviors is stronger for experience goods, by grouping products into physical experience goods and search goods. Table \ref{tab:table7} and \ref{tab:table8} presents the
...
\lstinputlisting[language=R, caption={Table D1 Generation}, label={lst:tableD1}, linerange={11-15}]{../table_D.do}
\pagebreak
...
\lstinputlisting[language=R, caption={Table D2 Generation}, label={lst:tableD2}, linerange={18-24}]{../table_D.do}
\pagebreak
...
\lstinputlisting[language=R, caption={Table D3 Generation}, label={lst:tableD3}, linerange={27-33}]{../table_D.do}
\pagebreak
...
\lstinputlisting[language=R, caption={Codes for GSC model}, label={codeGSC}, linerange={119-151, 156-177, 182-203, 207-224, 229-245, 250-266}]{../GSC_part.R}
...
In this section we focus on transaction level data and our treated group is determined by using a variable called \texttt{treatment} which takes value of 1 if \texttt{CCStorePresent} and \texttt{AfterStoreClosing} are 1 and takes value of 0 otherwise. We
use the dataset called zero-mile data which only includes transactions from zip code areas where a Circuit City store was closed for our causal forest models.
% There are some duplicate observations in these datasets therefore we decide to remove them from our analysis. We also create a third dataset combining these two and dropping the duplicate observations because there are some observations in zero-mile dataset we fail to observe in the five-mile dataset. \\
Codes for constructing treatment variable are listed below.
\lstinputlisting[language=R, caption={Constructing  Treatment Variable}, label={lst:treatment_var}, linerange={134-153}]{../celik_beyza_causal_forest.R}
We first investigate the treatment effects on Amazon sales using the transactions within the zip code where a Circuit City store was closed. For each transaction $i=1, \cdots, n$, we observe a binary treatment indicator \texttt{treatment} ($W_i$), a real valued outcome $\mathrm{\log}(\texttt{prod\_totprice})$ ($Y_i$), as well as 10 categorical covariates which are \texttt{hoh\_most\_education}, \texttt{census\_region}, \texttt{household\_size}, \texttt{hoh\_oldest\_age}, \texttt{children}, \texttt{racial\_background}, \texttt{connection\_speed}, \texttt{country\_of\_origin}, \texttt{prod\_category\_type} and \texttt{BBStorePresent}; and 4 real-valued covariates which are \texttt{pages\_viewed}, \texttt{duration}, \texttt{prod\_qty}, \texttt{household\_income}. We expanded out categorical random variables via one-hot encoding, thus resulting in covariates $X_i \in \mathbb{R}^p$  with $p = 38$ or $p = 37$ depending on the response variable.
We define causal effects via the potential outcomes model \citep{imbens2015causal}: For each sample $i$, the potential outcomes denoted by $Y_i(0)$ and $Y_i(1)$ corresponding to the outcome we would have observed  if the $i$-th sample was in control or treatment group, and assume that we observe $Y_i =Y_i(Wi)$.
...
Codes for estimating treatment effects on the logarithm of Amazon sales within the zip code where a Circuit City store was closed are listed below.
\lstinputlisting[language=R, caption={Estimating Treatment Effects on the Logarithm of Amazon Sales (Zero Mile) with Causal Forests}, label={lst:ama_cf_d1}, linerange={223-257}]{../celik_beyza_causal_forest.R}
We use the package \texttt{grf} \citep{tibshirani2018package} to apply causal forest on our data and also to estimate the average treatment effect on treated (ATT). The confidence interval for the average treatment effect is presented in Table \ref{tab:ama_cf1_att}.  Since the 90\% confidence interval do not include zero, we can say that the average treatment effect on treated is negative and statistically significant at the 0.1 level. Since the average treatment effect on treated is negative and significant we can say that the closure of a a close by Circuit City store might have caused a decrease on Amazon sales. 
% We also perform the same analysis when the radius is increased by five miles. Our findings are not significant therefore we do not report our results for the larger dataset.
\begin{table}[h]
	\caption{90\% CI for the ATT on $\mathrm{\log}$(\texttt{prod\_totprice}) for amazon.com (Zero Mile Data)}
	\vspace{1em}
	\label{tab:ama_cf1_att}
	\centering
	\begin{tabular}{rrr}
		\hline
		5\%  & $\hat{\tau_t}$ & 95\% \\ 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{pic/tauhat1_ama_hist.pdf}\caption{Histogram of out-of-bag estimates of CATE  on logarithm of xAmazon Sales (Zero-Mile Data)}
	\label{fig:tauhat1_ama_hist}
\end{figure}
As seen in Figure \ref{fig:tauhat1_ama_hist}, the causal forest CATE estimates exhibit variation; but this does not necessarily imply that $\tau^{-i} (X_i)$ (The $^{(-i)}$ superscripts denote "out-of-bag" predictions) is a better estimate of $\tau(X_i)$ than the overall average treatment effect estimate $\hat{\tau}$ that we obtain using the doubly robust approach (The package \textbf{grf} uses this approach) \citep{athey2019estimating}. We try a test for heterogeneity, motivated by the "best linear predictor" method of \cite{chernozhukov2018generic}, that fits the CATE as a linear function of the the out-of-bag causal forest estimates $\hat{\tau}^{-i}(X_i)$. The results of this test is presented in Table \ref{tab:avg_treat_hetero_cf1}. Since the differential forest prediction coefficient is significant and positive, we can say that the causal forest succeeded in finding heterogeneity.
\begin{table}[h] \centering 
	\caption{Best linear fit using forest predictions for CATE on logarithm of Amazon Sales (Zero-mile dataset)} 
	...
	\begin{tabular}{@{\extracolsep{5pt}}lc} 
		...
		\hline \\[-1.8ex] 
		mean.forest.prediction & 4.950 \\ 
		& (4.259) \\ 
		& \\ 
		differential.forest.prediction & 1.651$^{***}$ \\ 
		& (0.506) \\ 
		& \\ 
		\hline \\[-1.8ex] 
		\hline 
	...
	\end{tabular} 
\end{table}
Next, we consider the effect of store closure on customers' online shopping behaviors. We run two different causal forest using dependent variables $\mathrm{\log}$(\texttt{PagesPerDollar}) which is the logarithm of the number of pages viewed for every dollar worth of products either on amazon.com or bestbuy.com and $\mathrm{\log}(\texttt{MinsPerDollar})$ which is the logarithm of the minutes spent at the website for every purchase dollar.
We first investigate the treatment effect on $\mathrm{log}$(\texttt{PagesPerDollar}) using the amazon.com transactions within the zip code where a Circuit City store was closed. The average treatment effect on treated and the 95\% confidence interval is presented in Table (\ref{tab:ama_cf3_ate}) Since the 95\% confidence interval do not include zero we can say that the average treatment effect on treated ($\hat{\tau}_t$) is positive and statistically significant at the 0.05 level. Looking at this result we can say that the closure of Circuit City stores seems to cause an increase on the number of pages that amazon customers observe per a dollar worth of purchase. This result is consistent with the findings of \cite{samuel2020value}.
% We also perform the same analysis when the radius is increased by five miles. Our findings for five miles dataset are not significant therefore we do not report our results for the larger dataset.\\
\begin{table}[h]
	\caption{95\% CI for the ATT on $\mathrm{log}$(\texttt{PagesPerDollar}) for amazon.com (Zero Mile Data)}
	\vspace{1em}
	...
	\end{tabular}
\end{table}
Codes for estimating treatment effect on $\mathrm{log}$(\texttt{PagesPerDollar}) using the transactions within the zip code where a Circuit City store was closed are listed below.
\lstinputlisting[language=R, caption={Estimating Treatment Effects on $\mathrm{log}$(\texttt{PagesPerDollar}) for amazon.com (Zero Mile) with Causal Forests}, label={lst:ama_cf_d1}, linerange={345-393}]{../celik_beyza_causal_forest.R}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.45\linewidth]{pic/tauhat3_ama_hist.pdf}
	\caption{Histogram of out-of-bag estimates of CATE  on $\mathrm{log}$(\texttt{PagesPerDollar}) for amazon.com (Zero-Mile Data)}
	\label{fig:tauhat3_ama_hist}
\end{figure}

We can see in Figure \ref{fig:tauhat3_ama_hist}, the causal forest CATE estimates on $\mathrm{log}$(\texttt{PagesPerDollar}) exhibit variation. We test for heterogeneity and report its the results in Table \ref{tab:avg_treat_hetero_cf3}. We can say there is an evidence for heterogeneity because the differential forest prediction coefficient is significant and positive.
...
\begin{table}[h] \centering 
	...
	\begin{tabular}{@{\extracolsep{5pt}}lc} 
		...
		& \multicolumn{1}{c}{CATE} \\ 
		\hline \\[-1.8ex] 
		mean.forest.prediction & 2.804 \\ 
		& (3.577) \\ 
		& \\ 
		differential.forest.prediction & 1.288$^{***}$ \\ 
		& (0.308) \\ 
		& \\ 
		\hline \\[-1.8ex] 
		\hline 
...
Again to investigate the treatment effect on online search behaviors we use $\mathrm{log}$(\texttt{MinsPerDollar}) using the amazon.com transactions within the zip code where a Circuit City store was closed. The average treatment effect on treated and the 90\% confidence interval is presented in Table (\ref{tab:ama_cf5_ate}) Since the 90\% confidence interval includes zero we fail to find an evidence for the significance of the average treatment effect. This result is different from what is shown in \cite{samuel2020value}. We believe this comes from the fact that our dataset is actually a random subset of the original data that is used in the paper so our sample might not a good representation of that dataset. Since our estimate for the average treatment effect on treated is not significant we do not test the heterogeneity here but you can observe some heterogeneity in the estimated conditional average treatment effects from the Figure \ref{fig:tauhat3_ama_hist}.
% We also perform the same analysis when the radius is increased by five miles. Our findings for five miles dataset are not significant therefore we do not report our results for the larger dataset.\\

\begin{table}[h]
	\caption{90\% CI for the ATT on $\mathrm{log}$(\texttt{MinsPerDollar}) for amazon.com (Zero Mile Data)} 
...
Codes for estimating treatment effect on $\mathrm{log}$(\texttt{MinsPerDollar}) using the transactions within the zip code where a Circuit City store was closed are listed below
\lstinputlisting[language=R, caption={Estimating Treatment Effects on $\mathrm{\log}$(\texttt{PagesPerDollar}) for amazon.com (Zero Mile) with Causal Forests}, label={lst:ama_cf_d1}, linerange={475-520}]{../celik_beyza_causal_forest.R}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.4\linewidth]{pic/tauhat5_ama_hist.pdf}
	\caption{Histogram of out-of-bag estimates of CATE  on $\mathrm{log}$(\texttt{MinsPerDollar}) for amazon.com (Zero-Mile Data)}
	\label{fig:tauhat5_ama_hist}
\end{figure}

We do the same analyses using the Best Buy online transactions data but none of our casual forests estimations yields a significant results. We believe that since Best Buy owns physical stores, it didn't get affected by the closure of Circuit City stores as much as Amazon did. Our results for the transaction data from bestbuy.com is presented below. 

\begin{table}[H]
	\caption{90\% CI for the ATT on $\mathrm{\log}$(\texttt{prod\_totprice}) for bestbuy.com (Zero Mile Data)} 
	\vspace{1em}
	\label{tab:bb_cf1_att}
	\centering
	\begin{tabular}{rrr}
		\hline
		5\%  & $\hat{\tau_t}$ & 95\% \\ 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\linewidth]{pic/tauhat1_bb_hist.pdf}\caption{Histogram of out-of-bag estimates of CATE  on logarithm of Best Buy Online Sales (Zero-Mile Data)}
	\label{fig:tauhat1_bb_hist}
\end{figure}

\begin{table}[H]
	\caption{90\% CI for the ATT  on $\mathrm{log}$(\texttt{PagesPerDollar}) for bestbuy.com (Zero Mile Data)} 
	\vspace{1em}
	\label{tab:bb_cf3_att}
	\centering
	\begin{tabular}{rrr}
		\hline
		5\%  & $\hat{\tau_t}$ & 95\% \\ 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\linewidth]{pic/tauhat3_bb_hist.pdf}
	\caption{Histogram of out-of-bag estimates of CATE  on $\mathrm{log}$(\texttt{PagesPerDollar}) for bestbuy.com (Zero-Mile Data)}
	\label{fig:tauhat3_bb_hist}
\end{figure}

\begin{table}[H]
	\caption{90\% CI for the ATT  on $\mathrm{log}$(\texttt{MinsPerDollar}) for bestbuy.com (Zero Mile Data)} 
	\vspace{1em}
	\label{tab:bb_cf5_att}
	\centering
	\begin{tabular}{rrr}
		\hline
		5\%  & $\hat{\tau_t}$ & 95\% \\ 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\linewidth]{pic/tauhat5_bb_hist.pdf}
	\caption{Histogram of out-of-bag estimates of CATE  on $\mathrm{log}$(\texttt{MinsPerDollar}) for bestbuy.com (Zero-Mile Data)}
