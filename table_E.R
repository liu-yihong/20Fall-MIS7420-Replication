...
# Table E Data
data_0m_E <- sqldf("SELECT Zip_Code, MonthYear, domain_name, SUM(prod_totprice) AS TotalMonthlySales, SUM(pages_viewed) / SUM(prod_totprice) AS PagesPerDollar, SUM(duration) / SUM(prod_totprice) AS MinsPerDollar, AVG(CCStorePresent) AS CCStorePresent, AVG(BBStorePresent) AS BBStorePresent, AVG(AfterStoreClosing) AS AfterStoreClosing FROM concat_data1 GROUP BY Zip_Code, MonthYear, domain_name")
data_0m_E$DID <- data_0m_E$CCStorePresent * data_0m_E$AfterStoreClosing
data_0m_E$THREEINTER <- data_0m_E$DID * data_0m_E$BBStorePresent

# Table E1
staple.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "staples.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
walmart.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "walmart.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
dell.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "dell.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
cc.tE1.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "circuitcity.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")

stargazer(staple.tE1.0mile, walmart.tE1.0mile, dell.tE1.0mile,
          title="Results of the Sales Effect (All Product Categories)",
          align=TRUE, column.labels=c("staples.com-0 Mile","walmart.com-0 Mile","dell.com-0 Mile"),
          covariate.labels=c("beta_1", "beta_2"), no.space=TRUE, column.sep.width = "1pt", label = "tab:tabE1")

# Table E2
staple.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "staples.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
walmart.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "walmart.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
dell.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "dell.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
cc.tE2.0mile <- plm(log(PagesPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "circuitcity.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")

stargazer(staple.tE2.0mile, walmart.tE2.0mile, dell.tE2.0mile,
...
# Table E3
staple.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "staples.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
walmart.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "walmart.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
dell.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "dell.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
cc.tE3.0mile <- plm(log(MinsPerDollar + 1) ~ DID + THREEINTER, data = data_0m_E[data_0m_E$domain_name == "circuitcity.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")

stargazer(staple.tE3.0mile, walmart.tE3.0mile, dell.tE3.0mile,
          title="Results of the Online Search Effect (All Product Categories)",
          align=TRUE, column.labels=c("staples.com-0 Mile","walmart.com-0 Mile","dell.com-0 Mile"),
