...
# Table G Data
data_0m_tG <- sqldf("SELECT Zip_Code, MonthYear, domain_name, SUM(prod_totprice) AS TotalMonthlySales, AVG(CCStorePresent) AS CCStorePresent, AVG(BBStorePresent) AS BBStorePresent, AVG(AfterStoreClosing) AS AfterStoreClosing FROM concat_data1 GROUP BY Zip_Code, MonthYear, domain_name")
data_0m_tG$DID <- data_0m_tG$CCStorePresent * data_0m_tG$AfterStoreClosing
data_0m_tG$THREEINTER <- data_0m_tG$DID * data_0m_tG$BBStorePresent

# Table G
ama.tG.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tG[data_0m_tG$domain_name == "amazon.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
bb.tG.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tG[data_0m_tG$domain_name == "bestbuy.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
stp.tG.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tG[data_0m_tG$domain_name == "staples.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
walmart.tG.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tG[data_0m_tG$domain_name == "dell.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
cc.tG.0mile <- plm(log(TotalMonthlySales + 1) ~ DID + THREEINTER, data = data_0m_tG[data_0m_tG$domain_name == "circuitcity.com",], index = c("Zip_Code", "MonthYear"), model = "within", effect = "twoways")
